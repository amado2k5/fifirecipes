            ...
            <div className="[@media(max-height:420px)]:hidden flex flex-wrap items-center gap-4 text-xs text-stone-300 pt-2 mt-2 border-t border-white/10">
              {localized.prepTime && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>{t(`التحضير: ${localized.prepTime}`, `Prep: ${localized.prepTime}`, `Préparation : ${localized.prepTime}`, `Preparación: ${localized.prepTime}`, `下ごしらえ: ${localized.prepTime}`)}</span>
                </div>
              )}
              {localized.cookTime && (
                <div className="flex items-center gap-1.5">
                  <Flame className="w-3.5 h-3.5 text-amber-400" />
                  <span>{t(`الطهو: ${localized.cookTime}`, `Cook: ${localized.cookTime}`, `Cuisson : ${localized.cookTime}`, `Cocción: ${localized.cookTime}`, `調理: ${localized.cookTime}`)}</span>
                </div>
              )}
              {localized.servings && (
                <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-bold">
                      {t('خطوات الطهو الفريدة بعد تنقية وتوحيد الشروح', 'Step-by-step master cooking instructions', 'Instructions de cuisson détaillées, étape par étape', 'Instrucciones de cocina detalladas, paso a paso', 'ステップごとの詳しい調理手順')}
                    </p>
