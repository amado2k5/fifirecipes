        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="p-6 bg-stone-900 text-amber-100 rounded-2xl border border-amber-900/50 shadow-md">
            <div className="flex items-center gap-2 text-amber-400 text-xs font-bold uppercase tracking-wider mb-2">
              ...
              <span>{t('لوحة الوفاء والتخليد', 'Memorial Dedication Plaque', 'Plaque Commémorative', 'Placa Conmemorativa', '追悼のプレート', 'स्मृति समर्पण पट्टिका', 'Placa Comemorativa', 'Мемориальная посвятительная табличка', '纪念献辞牌匾', 'Gedenktafel zu Ehren', 'Targa Commemorativa', 'Αναμνηστική Πλάκα Αφιέρωσης', 'یادگاری تختی', 'لوح یادبود', 'Anma Plaketi')}</span>
            </div>
            ...
            {FATMA_BIOGRAPHY.milestones.map((ms, idx) => (
              <div key={idx} className="relative group">
                ...
                <h4 className="text-sm font-bold text-stone-900 mt-1">
                  {isAr ? ms.titleAr : isFr ? ms.titleFr : isEs ? ms.titleEs : isJa ? ms.titleJa : isHi ? ms.titleHi : isPt ? ms.titlePt : isRu ? ms.titleRu : isZh ? ms.titleZh : isDe ? ms.titleDe : isIt ? ms.titleIt : isEl ? ms.titleEl : isUr ? ms.titleUr : isFa ? ms.titleFa : isTr ? ms.titleTr : ms.titleEn}
                </h4>
                <p className="text-xs text-stone-600 mt-1 leading-relaxed">
                  {isAr ? ms.descAr : isFr ? ms.descFr : isEs ? ms.descEs : isJa ? ms.descJa : isHi ? ms.descHi : isPt ? ms.descPt : isRu ? ms.descRu : isZh ? ms.descZh : isDe ? ms.descDe : isIt ? ms.descIt : isEl ? ms.descEl : isUr ? ms.descUr : isFa ? ms.descFa : isTr ? ms.descTr : ms.descEn}
                </p>
              </div>
            ))}
          </div>
