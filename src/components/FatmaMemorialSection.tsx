...
export const FatmaMemorialSection: React.FC<FatmaMemorialSectionProps> = ({ lang, onOpenTribute }) => {
  ...
  const getMilestoneYear = (year: string) => {
    if (isAr) return year;
    if (year === 'حوالي 1961') return isFr ? 'v. 1961' : isEs ? 'c. 1961' : isJa ? '1961年頃' : isHi ? 'लगभग 1961' : isPt ? 'c. 1961' : isRu ? 'около 1961 г.' : isZh ? '1961年左右' : isDe ? 'um 1961' : isIt ? 'c. 1961' : isEl ? 'περ. 1961' : isUr ? 'تقریباً 1961' : isFa ? 'حدود 1961' : 'c. 1961';
    if (year === '8 مايو 2026') return isFr ? '8 mai 2026' : isEs ? '8 de mayo de 2026' : isJa ? '2026年5月8日' : isHi ? '8 मई 2026' : isPt ? '8 de maio de 2026' : isRu ? '8 мая 2026 г.' : isZh ? '2026年5月8日' : isDe ? '8. Mai 2026' : isIt ? '8 maggio 2026' : isEl ? '8 Μαΐου 2026' : isUr ? '8 مئی 2026' : isFa ? '8 مه 2026' : 'May 8, 2026';
    return year;
  };
  const name = isAr ? FATMA_BIOGRAPHY.nameAr : isFr ? FATMA_BIOGRAPHY.nameFr : isEs ? FATMA_BIOGRAPHY.nameEs : isJa ? FATMA_BIOGRAPHY.nameJa : isHi ? FATMA_BIOGRAPHY.nameHi : isPt ? FATMA_BIOGRAPHY.namePt : isRu ? FATMA_BIOGRAPHY.nameRu : isZh ? FATMA_BIOGRAPHY.nameZh : isDe ? FATMA_BIOGRAPHY.nameDe : isIt ? FATMA_BIOGRAPHY.nameIt : isEl ? FATMA_BIOGRAPHY.nameEl : isUr ? FATMA_BIOGRAPHY.nameUr : isFa ? FATMA_BIOGRAPHY.nameFa : FATMA_BIOGRAPHY.nameEn;
  const title = isAr ? FATMA_BIOGRAPHY.titleAr : isFr ? FATMA_BIOGRAPHY.titleFr : isEs ? FATMA_BIOGRAPHY.titleEs : isJa ? FATMA_BIOGRAPHY.titleJa : isHi ? FATMA_BIOGRAPHY.titleHi : isPt ? FATMA_BIOGRAPHY.titlePt : isRu ? FATMA_BIOGRAPHY.titleRu : isZh ? FATMA_BIOGRAPHY.titleZh : isDe ? FATMA_BIOGRAPHY.titleDe : isIt ? FATMA_BIOGRAPHY.titleIt : isEl ? FATMA_BIOGRAPHY.titleEl : isUr ? FATMA_BIOGRAPHY.titleUr : isFa ? FATMA_BIOGRAPHY.titleFa : FATMA_BIOGRAPHY.titleEn;
  const tagline = isAr ? FATMA_BIOGRAPHY.taglineAr : isFr ? FATMA_BIOGRAPHY.taglineFr : isEs ? FATMA_BIOGRAPHY.taglineEs : isJa ? FATMA_BIOGRAPHY.taglineJa : isHi ? FATMA_BIOGRAPHY.taglineHi : isPt ? FATMA_BIOGRAPHY.taglinePt : isRu ? FATMA_BIOGRAPHY.taglineRu : isZh ? FATMA_BIOGRAPHY.taglineZh : isDe ? FATMA_BIOGRAPHY.taglineDe : isIt ? FATMA_BIOGRAPHY.taglineIt : isEl ? FATMA_BIOGRAPHY.taglineEl : isUr ? FATMA_BIOGRAPHY.taglineUr : isFa ? FATMA_BIOGRAPHY.taglineFa : FATMA_BIOGRAPHY.taglineEn;
  const philosophy = isAr ? FATMA_BIOGRAPHY.philosophyAr : isFr ? FATMA_BIOGRAPHY.philosophyFr : isEs ? FATMA_BIOGRAPHY.philosophyEs : isJa ? FATMA_BIOGRAPHY.philosophyJa : isHi ? FATMA_BIOGRAPHY.philosophyHi : isPt ? FATMA_BIOGRAPHY.philosophyPt : isRu ? FATMA_BIOGRAPHY.philosophyRu : isZh ? FATMA_BIOGRAPHY.philosophyZh : isDe ? FATMA_BIOGRAPHY.philosophyDe : isIt ? FATMA_BIOGRAPHY.philosophyIt : isEl ? FATMA_BIOGRAPHY.philosophyEl : isUr ? FATMA_BIOGRAPHY.philosophyUr : isFa ? FATMA_BIOGRAPHY.philosophyFa : FATMA_BIOGRAPHY.philosophyEn;
  const biography = isAr ? FATMA_BIOGRAPHY.biographyAr : isFr ? FATMA_BIOGRAPHY.biographyFr : isEs ? FATMA_BIOGRAPHY.biographyEs : isJa ? FATMA_BIOGRAPHY.biographyJa : isHi ? FATMA_BIOGRAPHY.biographyHi : isPt ? FATMA_BIOGRAPHY.biographyPt : isRu ? FATMA_BIOGRAPHY.biographyRu : isZh ? FATMA_BIOGRAPHY.biographyZh : isDe ? FATMA_BIOGRAPHY.biographyDe : isIt ? FATMA_BIOGRAPHY.biographyIt : isEl ? FATMA_BIOGRAPHY.biographyEl : isUr ? FATMA_BIOGRAPHY.biographyUr : isFa ? FATMA_BIOGRAPHY.biographyFa : FATMA_BIOGRAPHY.biographyEn;
  const memorialPlaque = isAr ? FATMA_BIOGRAPHY.memorialPlaqueAr : isFr ? FATMA_BIOGRAPHY.memorialPlaqueFr : isEs ? FATMA_BIOGRAPHY.memorialPlaqueEs : isJa ? FATMA_BIOGRAPHY.memorialPlaqueJa : isHi ? FATMA_BIOGRAPHY.memorialPlaqueHi : isPt ? FATMA_BIOGRAPHY.memorialPlaquePt : isRu ? FATMA_BIOGRAPHY.memorialPlaqueRu : isZh ? FATMA_BIOGRAPHY.memorialPlaqueZh : isDe ? FATMA_BIOGRAPHY.memorialPlaqueDe : isIt ? FATMA_BIOGRAPHY.memorialPlaqueIt : isEl ? FATMA_BIOGRAPHY.memorialPlaqueEl : isUr ? FATMA_BIOGRAPHY.memorialPlaqueUr : isFa ? FATMA_BIOGRAPHY.memorialPlaqueFa : FATMA_BIOGRAPHY.memorialPlaqueEn;
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-6 border-t border-stone-800 text-stone-300 text-xs sm:text-sm">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="text-[11px] text-stone-400">{t('أستاذة العزف على البيانو', 'Piano Professor', 'Professeure de Piano', 'Profesora de Piano', 'ピアノ教授', 'पियानो प्रोफेसर', 'Professora de Piano', 'Профессор фортепиано', '钢琴教授', 'Klavierprofessorin', 'Professoressa di Pianoforte', 'Καθηγήτρια Πιάνου', 'پیانو کی پروفیسر', 'استاد پیانو')}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 col-span-2 sm:col-span-1">
              ...
              <div>
                <div className="font-bold text-white">{t('20 عاماً من البحث', '20 Years of Research', '20 Années de Recherche', '20 Años de Investigación', '20年間の研究', '20 वर्षों का शोध', '20 Anos de Pesquisa', '20 лет исследований', '20年研究历程', '20 Jahre Forschung', '20 Anni di Ricerca', '20 Χρόνια Έρευνας', '20 سال کی تحقیق', '20 سال پژوهش')}</div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="p-6 bg-stone-900 text-amber-100 rounded-2xl border border-amber-900/50 shadow-md">
            <div className="flex items-center gap-2 text-amber-400 text-xs font-bold uppercase tracking-wider mb-2">
              ...
              <span>{t('لوحة الوفاء والتخليد', 'Memorial Dedication Plaque', 'Plaque Commémorative', 'Placa Conmemorativa', '追悼のプレート', 'स्मृति समर्पण पट्टिका', 'Placa Comemorativa', 'Мемориальная посвятительная табличка', '纪念献辞牌匾', 'Gedenktafel zu Ehren', 'Targa Commemorativa', 'Αναμνηστική Πλάκα Αφιέρωσης', 'یادگاری تختی', 'لوح یادبود')}</span>
            </div>
            ...
            {FATMA_BIOGRAPHY.milestones.map((ms, idx) => (
              <div key={idx} className="relative group">
                ...
                <h4 className="text-sm font-bold text-stone-900 mt-1">
                  {isAr ? ms.titleAr : isFr ? ms.titleFr : isEs ? ms.titleEs : isJa ? ms.titleJa : isHi ? ms.titleHi : isPt ? ms.titlePt : isRu ? ms.titleRu : isZh ? ms.titleZh : isDe ? ms.titleDe : isIt ? ms.titleIt : isEl ? ms.titleEl : isUr ? ms.titleUr : isFa ? ms.titleFa : ms.titleEn}
                </h4>
                <p className="text-xs text-stone-600 mt-1 leading-relaxed">
                  {isAr ? ms.descAr : isFr ? ms.descFr : isEs ? ms.descEs : isJa ? ms.descJa : isHi ? ms.descHi : isPt ? ms.descPt : isRu ? ms.descRu : isZh ? ms.descZh : isDe ? ms.descDe : isIt ? ms.descIt : isEl ? ms.descEl : isUr ? ms.descUr : isFa ? ms.descFa : ms.descEn}
                </p>
              </div>
            ))}
          </div>
