...
const CATEGORY_NAMES: Record<MasterIngredient['category'], { ar: string; en: string }> = {
  meat_poultry: { ar: 'لحوم ودواجن', en: 'Meat & Poultry' },
  seafood: { ar: 'أسماك ومأكولات بحرية', en: 'Seafood' },
  vegetable: { ar: 'خضروات وأعشاب', en: 'Vegetables & Herbs' },
  dairy_fat: { ar: 'ألبان ودهون', en: 'Dairy & Fats' },
  grain_starch: { ar: 'حبوب ونشويات', en: 'Grains & Starches' },
  spice_seasoning: { ar: 'توابل وبهارات', en: 'Spices & Seasoning' },
  sweet_fruit: { ar: 'سكريات وفواكه', en: 'Sweets & Fruits' },
  liquid: { ar: 'سوائل ومشروبات', en: 'Liquids & Broths' },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-stone-200/80 p-5 sm:p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <h2 className="text-xl sm:text-2xl font-bold text-stone-900 flex items-center gap-2.5">
              ...
              <span>{isAr ? 'سجل المكونات الموحدة (Master Ingredients Registry)' : 'Master Ingredients Registry'}</span>
            </h2>
          ...
          <div className="bg-amber-50 px-4 py-2 rounded-xl border border-amber-200 text-amber-900 text-xs sm:text-sm font-semibold">
            <span>{isAr ? 'إجمالي المكونات الموحدة:' : 'Total Master Ingredients:'}</span>{' '}
        ...
        <div className="mt-5 pt-5 border-t border-stone-100 flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder={isAr ? 'ابحث عن مكون (مثل: سمن بلدي، حبهان...)' : 'Search ingredient (e.g. Cardamom, Ghee)...'}
              value={searchTerm}
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2">
                <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-stone-100 text-stone-600 border border-stone-200">
                  {CATEGORY_NAMES[item.category]?.[isAr ? 'ar' : 'en'] || item.category}
                </span>

                <span className="text-[11px] font-bold text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
                  {isAr ? `${item.recipesCount} وصفات` : `${item.recipesCount} recipes`}
                </span>
              </div>
            ...
            <div className="mt-3 pt-3 border-t border-stone-100">
              <span className="text-[11px] text-stone-500 font-semibold block mb-1.5">
                {isAr ? 'الوصفات التي تستخدمه:' : 'Used in recipes:'}
              </span>
      ...
      {filteredItems.length === 0 && (
        <div className="text-center py-12 bg-white rounded-2xl border border-stone-200 text-stone-500">
          ...
          <p className="font-semibold text-sm">{isAr ? 'لا توجد مكونات مطابقة لبحثك' : 'No ingredients match your query'}</p>
        </div>
      )}
    </div>
  );
