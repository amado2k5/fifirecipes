...
const CATEGORY_NAMES: Record<MasterIngredient['category'], { ar: string; en: string; fr: string; es: string; ja: string; hi: string; pt: string; ru: string; zh: string; de: string; it: string; el: string; ur: string; fa: string; tr: string; ku: string }> = {
  meat_poultry: { ar: 'لحوم ودواجن', en: 'Meat & Poultry', fr: 'Viandes et Volailles', es: 'Carnes y Aves', ja: '肉と鶏肉', hi: 'मांस और मुर्ग़', pt: 'Carnes e Aves', ru: 'Мясо и Птица', zh: '肉类与禽类', de: 'Fleisch & Geflügel', it: 'Carne e Pollame', el: 'Κρέας & Πουλερικά', ur: 'گوشت اور مرغی', fa: 'گوشت و مرغ', tr: 'Et ve Tavuk', ku: 'Goşt û Mirîşk' },
  seafood: { ar: 'أسماك ومأكولات بحرية', en: 'Seafood', fr: 'Fruits de Mer', es: 'Mariscos', ja: '魚介類', hi: 'समुद्री भोजन', pt: 'Frutos do Mar', ru: 'Морепродукты', zh: '海鲜', de: 'Meeresfrüchte', it: 'Frutti di Mare', el: 'Θαλασσινά', ur: 'سمندری غذا', fa: 'غذاهای دریایی', tr: 'Deniz Ürünleri', ku: 'Berhemên Deryayê' },
  vegetable: { ar: 'خضروات وأعشاب', en: 'Vegetables & Herbs', fr: 'Légumes et Herbes', es: 'Verduras y Hierbas', ja: '野菜とハーブ', hi: 'सब्ज़ियां और जड़ी-बूटियां', pt: 'Vegetais e Ervas', ru: 'Овощи и Травы', zh: '蔬菜与香草', de: 'Gemüse & Kräuter', it: 'Verdure ed Erbe', el: 'Λαχανικά & Βότανα', ur: 'سبزیاں اور جڑی بوٹیاں', fa: 'سبزیجات و گیاهان معطر', tr: 'Sebzeler ve Otlar', ku: 'Sebze û Giya' },
  dairy_fat: { ar: 'ألبان ودهون', en: 'Dairy & Fats', fr: 'Produits Laitiers et Matières Grasses', es: 'Lácteos y Grasas', ja: '乳製品と油脂', hi: 'डेयरी और वसा', pt: 'Laticínios e Gorduras', ru: 'Молочные продукты и Жиры', zh: '乳制品与油脂', de: 'Milchprodukte & Fette', it: 'Latticini e Grassi', el: 'Γαλακτοκομικά & Λιπαρά', ur: 'دودھ کی مصنوعات اور چکنائی', fa: 'لبنیات و چربی‌ها', tr: 'Süt Ürünleri ve Yağlar', ku: 'Berhemên Şîr û Rûn' },
  grain_starch: { ar: 'حبوب ونشويات', en: 'Grains & Starches', fr: 'Céréales et Féculents', es: 'Granos y Féculas', ja: '穀物と主食', hi: 'अनाज और स्टार्च', pt: 'Grãos e Amidos', ru: 'Крупы и Крахмалистые продукты', zh: '谷物与主食', de: 'Getreide & Sättigungsbeilagen', it: 'Cereali e Amidi', el: 'Δημητριακά & Αμυλούχα', ur: 'اناج اور نشاستہ', fa: 'غلات و نشاسته‌ها', tr: 'Tahıllar ve Nişastalılar', ku: 'Dexl û Xwarinên Nîşasteyî' },
  ...
  sweet_fruit: { ar: 'سكريات وفواكه', en: 'Sweets & Fruits', fr: 'Sucreries et Fruits', es: 'Dulces y Frutas', ja: '甘味料と果物', hi: 'मिठास और फल', pt: 'Doces e Frutas', ru: 'Сладости и Фрукты', zh: '糖类与水果', de: 'Süßes & Obst', it: 'Dolci e Frutta', el: 'Γλυκά & Φρούτα', ur: 'مٹھاس اور پھل', fa: 'شیرینی‌ها و میوه‌ها', tr: 'Tatlandırıcılar ve Meyveler', ku: 'Şêrînker û Fêkî' },
  liquid: { ar: 'سوائل ومشروبات', en: 'Liquids & Broths', fr: 'Liquides et Bouillons', es: 'Líquidos y Caldos', ja: '液体とだし', hi: 'तरल और शोरबा', pt: 'Líquidos e Caldos', ru: 'Жидкости и Бульоны', zh: '汤汁与高汤', de: 'Flüssigkeiten & Brühen', it: 'Liquidi e Brodi', el: 'Υγρά & Ζωμοί', ur: 'مائعات اور یخنی', fa: 'مایعات و آب‌گوشت', tr: 'Sıvılar ve Et Suları', ku: 'Şilav û Avgoşt' },
  other: { ar: 'أخرى', en: 'Other', fr: 'Autres', es: 'Otros', ja: 'その他', hi: 'अन्य', pt: 'Outros', ru: 'Прочее', zh: '其他', de: 'Sonstiges', it: 'Altro', el: 'Άλλα', ur: 'دیگر', fa: 'سایر', tr: 'Diğer', ku: 'Yên Din' }
};
...
export const MasterIngredientsView: React.FC<MasterIngredientsViewProps> = ({
  ...
  const isTr = lang === 'tr';
  const isKu = lang === 'ku';
  const langKey: 'ar' | 'en' | 'fr' | 'es' | 'ja' | 'hi' | 'pt' | 'ru' | 'zh' | 'de' | 'it' | 'el' | 'ur' | 'fa' | 'tr' | 'ku' = isAr ? 'ar' : isFr ? 'fr' : isEs ? 'es' : isJa ? 'ja' : isHi ? 'hi' : isPt ? 'pt' : isRu ? 'ru' : isZh ? 'zh' : isDe ? 'de' : isIt ? 'it' : isEl ? 'el' : isUr ? 'ur' : isFa ? 'fa' : isTr ? 'tr' : isKu ? 'ku' : 'en';
      ...
      <div className="bg-white rounded-2xl border border-stone-200/80 p-5 sm:p-6 shadow-xs">
        ...
        <div className="mt-5 pt-5 border-t border-stone-100 flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto w-full pb-1 scrollbar-none">
            ...
            >
              {t('الكل', 'All', 'Tout', 'Todos', 'すべて', 'सभी', 'Todos', 'Все', '全部', 'Alle', 'Tutti', 'Όλα', 'سب', 'همه', 'Tümü', 'Hemû')} ({registry.length})
            </button>
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-[11px] font-bold text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
                  {t(`${item.recipesCount} وصفات`, `${item.recipesCount} recipes`, `${item.recipesCount} recettes`, `${item.recipesCount} recetas`, `${item.recipesCount}件のレシピ`, `${item.recipesCount} व्यंजन`, `${item.recipesCount} receitas`, `${item.recipesCount} рецептов`, `${item.recipesCount}个食谱`, `${item.recipesCount} Rezepte`, `${item.recipesCount} ricette`, `${item.recipesCount} συνταγές`, `${item.recipesCount} ترکیبیں`, `${item.recipesCount} دستور`, `${item.recipesCount} tarif`, `${item.recipesCount} reçete`)}
                </span>
              </div>
            ...
            <div className="mt-3 pt-3 border-t border-stone-100">
              <span className="text-[11px] text-stone-500 font-semibold block mb-1.5">
                {t('الوصفات التي تستخدمه:', 'Used in recipes:', 'Utilisé dans les recettes :', 'Usado en las recetas:', '使用しているレシピ:', 'जिन व्यंजनों में उपयोग होता है:', 'Usado nas receitas:', 'Используется в рецептах:', '使用于以下食谱：', 'Verwendet in Rezepten:', 'Utilizzato nelle ricette:', 'Χρησιμοποιείται στις συνταγές:', 'ان ترکیبوں میں استعمال:', 'به‌کاررفته در دستورهای:', 'Kullanıldığı tarifler:', 'Di van reçeteyan de tê bikaranîn:')}
              </span>
