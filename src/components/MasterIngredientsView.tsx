...
const CATEGORY_NAMES: Record<MasterIngredient['category'], { ar: string; en: string; fr: string; es: string; ja: string; hi: string; pt: string; ru: string; zh: string; de: string; it: string; el: string; ur: string }> = {
  meat_poultry: { ar: 'لحوم ودواجن', en: 'Meat & Poultry', fr: 'Viandes et Volailles', es: 'Carnes y Aves', ja: '肉と鶏肉', hi: 'मांस और मुर्ग़', pt: 'Carnes e Aves', ru: 'Мясо и Птица', zh: '肉类与禽类', de: 'Fleisch & Geflügel', it: 'Carne e Pollame', el: 'Κρέας & Πουλερικά', ur: 'گوشت اور مرغی' },
  seafood: { ar: 'أسماك ومأكولات بحرية', en: 'Seafood', fr: 'Fruits de Mer', es: 'Mariscos', ja: '魚介類', hi: 'समुद्री भोजन', pt: 'Frutos do Mar', ru: 'Морепродукты', zh: '海鲜', de: 'Meeresfrüchte', it: 'Frutti di Mare', el: 'Θαλασσινά', ur: 'سمندری غذا' },
  vegetable: { ar: 'خضروات وأعشاب', en: 'Vegetables & Herbs', fr: 'Légumes et Herbes', es: 'Verduras y Hierbas', ja: '野菜とハーブ', hi: 'सब्ज़ियां और जड़ी-बूटियां', pt: 'Vegetais e Ervas', ru: 'Овощи и Травы', zh: '蔬菜与香草', de: 'Gemüse & Kräuter', it: 'Verdure ed Erbe', el: 'Λαχανικά & Βότανα', ur: 'سبزیاں اور جڑی بوٹیاں' },
  dairy_fat: { ar: 'ألبان ودهون', en: 'Dairy & Fats', fr: 'Produits Laitiers et Matières Grasses', es: 'Lácteos y Grasas', ja: '乳製品と油脂', hi: 'डेयरी और वसा', pt: 'Laticínios e Gorduras', ru: 'Молочные продукты и Жиры', zh: '乳制品与油脂', de: 'Milchprodukte & Fette', it: 'Latticini e Grassi', el: 'Γαλακτοκομικά & Λιπαρά', ur: 'دودھ کی مصنوعات اور چکنائی' },
  grain_starch: { ar: 'حبوب ونشويات', en: 'Grains & Starches', fr: 'Céréales et Féculents', es: 'Granos y Féculas', ja: '穀物と主食', hi: 'अनाज और स्टार्च', pt: 'Grãos e Amidos', ru: 'Крупы и Крахмалистые продукты', zh: '谷物与主食', de: 'Getreide & Sättigungsbeilagen', it: 'Cereali e Amidi', el: 'Δημητριακά & Αμυλούχα', ur: 'اناج اور نشاستہ' },
  spice_seasoning: { ar: 'توابل وبهارات', en: 'Spices & Seasoning', fr: 'Épices et Assaisonnements', es: 'Especias y Condimentos', ja: 'スパイスと調味料', hi: 'मसाले और सीज़निंग', pt: 'Especiarias e Temperos', ru: 'Специи и Приправы', zh: '香料与调味料', de: 'Gewürze & Würzmittel', it: 'Spezie e Condimenti', el: 'Μπαχαρικά & Καρυκεύματα', ur: 'مصالحے اور ذائقے' },
  sweet_fruit: { ar: 'سكريات وفواكه', en: 'Sweets & Fruits', fr: 'Sucreries et Fruits', es: 'Dulces y Frutas', ja: '甘味料と果物', hi: 'मिठास और फल', pt: 'Doces e Frutas', ru: 'Сладости и Фрукты', zh: '糖类与水果', de: 'Süßes & Obst', it: 'Dolci e Frutta', el: 'Γλυκά & Φρούτα', ur: 'مٹھاس اور پھل' },
  liquid: { ar: 'سوائل ومشروبات', en: 'Liquids & Broths', fr: 'Liquides et Bouillons', es: 'Líquidos y Caldos', ja: '液体とだし', hi: 'तरल और शोरबा', pt: 'Líquidos e Caldos', ru: 'Жидкости и Бульоны', zh: '汤汁与高汤', de: 'Flüssigkeiten & Brühen', it: 'Liquidi e Brodi', el: 'Υγρά & Ζωμοί', ur: 'مائعات اور یخنی' },
  other: { ar: 'أخرى', en: 'Other', fr: 'Autres', es: 'Otros', ja: 'その他', hi: 'अन्य', pt: 'Outros', ru: 'Прочее', zh: '其他', de: 'Sonstiges', it: 'Altro', el: 'Άλλα', ur: 'دیگر' }
};
...
export const MasterIngredientsView: React.FC<MasterIngredientsViewProps> = ({
  ...
  const isEl = lang === 'el';
  const isUr = lang === 'ur';
  const langKey: 'ar' | 'en' | 'fr' | 'es' | 'ja' | 'hi' | 'pt' | 'ru' | 'zh' | 'de' | 'it' | 'el' | 'ur' = isAr ? 'ar' : isFr ? 'fr' : isEs ? 'es' : isJa ? 'ja' : isHi ? 'hi' : isPt ? 'pt' : isRu ? 'ru' : isZh ? 'zh' : isDe ? 'de' : isIt ? 'it' : isEl ? 'el' : isUr ? 'ur' : 'en';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-stone-200/80 p-5 sm:p-6 shadow-xs">
        ...
        <div className="mt-5 pt-5 border-t border-stone-100 flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto w-full pb-1 scrollbar-none">
            ...
            >
              {t('الكل', 'All', 'Tout', 'Todos', 'すべて', 'सभी', 'Todos', 'Все', '全部', 'Alle', 'Tutti', 'Όλα', 'سب')} ({registry.length})
            </button>
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-[11px] font-bold text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
                  {t(`${item.recipesCount} وصفات`, `${item.recipesCount} recipes`, `${item.recipesCount} recettes`, `${item.recipesCount} recetas`, `${item.recipesCount}件のレシピ`, `${item.recipesCount} व्यंजन`, `${item.recipesCount} receitas`, `${item.recipesCount} рецептов`, `${item.recipesCount}个食谱`, `${item.recipesCount} Rezepte`, `${item.recipesCount} ricette`, `${item.recipesCount} συνταγές`, `${item.recipesCount} ترکیبیں`)}
                </span>
              </div>
            ...
            <div className="mt-3 pt-3 border-t border-stone-100">
              <span className="text-[11px] text-stone-500 font-semibold block mb-1.5">
                {t('الوصفات التي تستخدمه:', 'Used in recipes:', 'Utilisé dans les recettes :', 'Usado en las recetas:', '使用しているレシピ:', 'जिन व्यंजनों में उपयोग होता है:', 'Usado nas receitas:', 'Используется в рецептах:', '使用于以下食谱：', 'Verwendet in Rezepten:', 'Utilizzato nelle ricette:', 'Χρησιμοποιείται στις συνταγές:', 'ان ترکیبوں میں استعمال:')}
              </span>
      ...
      {filteredItems.length === 0 && (
        <div className="text-center py-12 bg-white rounded-2xl border border-stone-200 text-stone-500">
          ...
          <p className="font-semibold text-sm">{t('لا توجد مكونات مطابقة لبحثك', 'No ingredients match your query', 'Aucun ingrédient ne correspond à votre recherche', 'Ningún ingrediente coincide con tu búsqueda', '検索条件に一致する食材が見つかりませんでした', 'आपकी खोज से मेल खाने वाली कोई सामग्री नहीं मिली', 'Nenhum ingrediente corresponde à sua busca', 'Ингредиенты по вашему запросу не найдены', '没有找到匹配的食材', 'Keine Zutaten entsprechen Ihrer Suche', 'Nessun ingrediente corrisponde alla tua ricerca', 'Κανένα υλικό δεν ταιριάζει με την αναζήτησή σας', 'آپ کی تلاش سے کوئی جزو نہیں ملا')}</p>
        </div>
      )}
    </div>
  );
