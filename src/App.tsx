...
export default function App() {
  ...
  const handleShareSite = async () => {
    ...
    if (navigator.share) {
      ...
      setNotification({ message: t('تم نسخ رابط الموقع', 'Site link copied', 'Lien du site copié', 'Enlace del sitio copiado', 'サイトのリンクをコピーしました', 'साइट लिंक कॉपी हो गया'), type: 'success' });
  ...
  const handleShareRecipe = async (recipe: Recipe) => {
    ...
    if (result.copied) {
      setNotification({ message: t('تم نسخ رابط الوصفة', 'Recipe link copied', 'Lien de la recette copié', 'Enlace de la receta copiado', 'レシピのリンクをコピーしました', 'रेसिपी लिंक कॉपी हो गया'), type: 'success' });
      setTimeout(() => setNotification(null), 3500);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            >
              ...
              <span>{t('أسئلة أو ملاحظات أو مشاكل؟ أرسل ملاحظاتك', 'Questions, comments, or issues? Submit feedback', 'Questions, commentaires ou problèmes ? Envoyez vos remarques', '¿Preguntas, comentarios o problemas? Envía tus comentarios', 'ご質問・ご意見・不具合の報告はこちら', 'सवाल, टिप्पणी या समस्या? प्रतिक्रिया भेजें')}</span>
            </button>
          </div>
          ...
          <div className="pt-4 border-t border-stone-100 flex flex-col sm:flex-row items-center justify-between text-[11px] text-stone-400 gap-2">
            ...
            <div className="flex items-center gap-2">
              <span>{t('العربية والإنجليزية والفرنسية والإسبانية واليابانية والهندية مدعومة', 'Arabic, English, French, Spanish, Japanese, and Hindi supported', 'Arabe, anglais, français, espagnol, japonais et hindi pris en charge', 'Árabe, inglés, francés, español, japonés e hindi disponibles', 'アラビア語・英語・フランス語・スペイン語・日本語・ヒンディー語に対応', 'अरबी, अंग्रेज़ी, फ़्रेंच, स्पेनिश, जापानी और हिन्दी उपलब्ध')}</span>
              <span>•</span>
              <span>{t('مشاركة الوصفة الفردية مفعّلة', 'Single-Recipe Sharing Enabled', 'Partage de Recette Individuelle Activé', 'Compartir Receta Individual Habilitado', '個別レシピの共有が可能', 'एकल रेसिपी साझा करने की सुविधा उपलब्ध')}</span>
            </div>
          </div>
        </div>
      </footer>
